...
@export var small_radius: int = 100
@export var big_radius: int = 100
...
func on_small_timer_timeout():
	...
	var spawn_position = global_position + (random_direction * small_radius)
...
func on_big_timer_timeout():
	...
	var random_direction = Vector2.LEFT.rotated(randf_range(0, PI))
	var spawn_position = global_position + (random_direction * big_radius)
